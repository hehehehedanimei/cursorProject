    ...
    try {
       ...
       WHERE status IN ('completed', 'failed')`);
        ...
        for (const task of tasks) {
            // 获取任务的所有步骤
            const steps = await connection_1.Database.all('SELECT * FROM task_steps WHERE task_id = ? ORDER BY step_order', [task.id]);
            // 获取任务的所有消息
            const messages = await connection_1.Database.all('SELECT * FROM messages WHERE task_id = ? ORDER BY created_time', [task.id]);
            // 计算统计信息
...
router.get('/:taskId', async (req, res) => {
    try {
        ...
        // 获取消息信息
        const messages = await connection_1.Database.all('SELECT * FROM messages WHERE task_id = ? ORDER BY created_time', [taskId]);
        // 计算统计信息
        const totalSteps = steps.length;
        const completedSteps = steps.filter(s => s.status === 'completed').length;
        const failedSteps = steps.filter(s => s.status === 'failed').length;
