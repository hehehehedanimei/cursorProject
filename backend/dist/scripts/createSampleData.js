...
async function createSampleData() {
    try {
        ...
        for (const task of sampleTasks) {
            const result = await connection_1.Database.run(`INSERT INTO tasks (name, description, status, start_time, end_time, created_time, updated_time) 
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [task.name, task.description, task.status, task.start_time, task.end_time, task.created_time, task.updated_time]);
            ...
            for (const step of steps) {
                const stepResult = await connection_1.Database.run(`INSERT INTO task_steps (
            ...
            start_time, end_time, estimated_duration, actual_duration, created_time
                ...
                // 为每个步骤创建消息记录
                await connection_1.Database.run(`INSERT INTO messages (task_id, step_id, message_type, message_content, is_sent, created_time)
           VALUES (?, ?, ?, ?, ?, ?)`, [
                    taskId,
                    stepResult.lastID,
