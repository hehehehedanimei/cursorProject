...
async function importServices() {
    ...
    `;
        await connection_1.Database.run(createMessageTemplatesTable);
        console.log('消息模板表创建完成');
        const csvPath = path_1.default.join(__dirname, '../../../上线流程检查清单.csv');
        const services = parseCSV(csvPath);
        console.log(`解析到 ${services.length} 条服务配置`);
        // 清空现有服务数据
        await connection_1.Database.run('DELETE FROM services');
        // 插入新数据
      ...
      `;
        ...
        // 清空现有配置
        await connection_1.Database.run('DELETE FROM configs');
        for (const config of configs) {
            await connection_1.Database.run('INSERT INTO configs (config_key, config_value, description, updated_time) VALUES (?, ?, ?, ?)', [config.key, config.value, config.description, new Date().toISOString()]);
        }
        // 插入消息模板
        const templates = [
            {
        ...
        // 清空现有模板
        await connection_1.Database.run('DELETE FROM message_templates');
        for (const template of templates) {
            await connection_1.Database.run(`INSERT INTO message_templates (name, type, template, variables, is_active, created_time) 
         VALUES (?, ?, ?, ?, ?, ?)`, [
                template.name,
                template.type,
                template.template,
...
//# sourceMappingURL=importServices.js.map